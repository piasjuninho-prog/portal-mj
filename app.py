...
if st.session_state.perfil is None:
    ...
    # (Abas Gestão, Planos e Vincular mantidas v90.0)
    if menu == "🏫 Gestão":
    ...
    # --- 🏠 DASHBOARD (v117.0 - CORREÇÃO TOTAL DE BRUTO) ---
    elif menu == "🏠 Dashboard":
        ...
        try:
            ...
            if v_raw:
                ...
                # Força bruto ser número para somar certo
                df_v['bruto_v'] = pd.to_numeric(df_v['bruto'], errors='coerce').fillna(0.0)
                ...
                df['lojista_final'] = df.apply(lambda x: x['nome_lojista'] if pd.notnull(x['nome_lojista']) else f"⚠️ NÃO VINCULADO ({x['key']})", axis=1)
