...
if st.session_state.perfil is None:
    ...
    if menu == "🏫 Gestão":
        ...
        if res_e.data and res_p.data:
            with st.form("vin"):
                c = st.selectbox("Cliente", sorted([str(e['nome_fantasia']) for e in res_e.data]))
                ns = st.text_input("NS da Maquina (ou Terminal)")
                pl = st.selectbox("Plano", sorted([str(p['nome_plano']) for p in res_p.data]))
    ...
    # --- 🏠 DASHBOARD (VERSÃO BLINDADA v73.0) ---
    elif menu == "🏠 Dashboard":
        ...
        try:
            ...
            if v_raw and m_raw:
                ...
                # Inner Join: Só mostra o que foi vinculado
                ...
                # --- TRATAMENTO DE DATA (CORREÇÃO DO BUG) ---
                df['data_dt'] = df['data_venda'].apply(converter_data_seguro)
                df = df.dropna(subset=['data_dt'])
                # Criamos uma coluna de data pura para o filtro
                df['data_only'] = pd.to_datetime(df['data_dt']).dt.date
            ...
            else: st.info("As vendas aparecerão aqui após vincular os NS na aba Vincular.")
        except Exception as e: st.error(f"Erro no Dashboard: {e}")

st.sidebar.caption("MJ Soluções v73.0")
