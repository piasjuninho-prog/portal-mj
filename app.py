...
if st.session_state.perfil is None:
    ...
    # --- 🏠 DASHBOARD (v109.0 - CORREÇÃO DE MERGE STR/INT) ---
    elif menu == "🏠 Dashboard":
        ...
        try:
            ...
            if v_raw and m_raw:
                ...
                # NORMALIZAÇÃO DE CHAVES PARA EVITAR ERRO STR/INT
                ...
                # Cruzamento Vendas + Máquinas
                df = pd.merge(df_v, df_m, left_on='link_key', right_on='ns_short', how='inner')
                
                # Cruzamento Planos (Forçando ID como String para o Merge)
                ...
                # Cruzamento Taxas
                df_t['id_plano_str'] = df_t['id_plano'].astype(str)
                ...
                df_t['ba_t'] = df_t['bandeira'].astype(str).str.strip().str.lower()
                df_t['me_t'] = df_t['meio'].astype(str).str.strip().str.lower()

                df = pd.merge(df, df_t, left_on=['id_p_str', 'ba_adj', 'pl_adj'], right_on=['id_plano_str', 'ba_t', 'me_t'], how='left')
                
                df['data_dt'] = df['data_venda'].apply(converter_data_seguro); df = df.dropna(subset=['data_dt'])

                # Filtros
                l_filt = sorted(df['nome_lojista'].astype(str).unique())
                if st.session_state.perfil == "admin":
                    esc = st.sidebar.multiselect("Lojistas:", l_filt, default=l_filt); df = df[df['nome_lojista'].isin(esc)]
                ...
                if not df.empty:
                    ...
                    if st.session_state.perfil == "admin": c4.metric("Lucro Real", f"R$ {df['lucro'].sum():,.2f}")
...
st.sidebar.caption("MJ Soluções v109.0")
