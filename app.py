...
if not st.session_state.auth:
    ...
    # --- ABA VINCULAR ---
        ...
        with st.form("vinc"):
            ...
            ns_txt = st.text_area("Números de Série (NS) - Separe por vírgula")
    ...
    # --- ABA DASHBOARD (v202.0 - RAIO-X DE VÍNCULOS) ---
    elif menu == "🏠 Dashboard":
        st_autorefresh(interval=60000, key="ref")
        st.title("📊 Dashboard")
        d_sel = st.sidebar.date_input("Data do Filtro", date(2026, 8, 7))
        ...
        if v_res.data:
            ...
            df['nome_lojista'] = df['nome_lojista'].fillna("⚠️ MÁQUINA NÃO VINCULADA")

            # --- PAINEL DE ALERTA DE MÁQUINAS PERDIDAS ---
            perdidos = df[df['nome_lojista'] == "⚠️ MÁQUINA NÃO VINCULADA"]['link'].unique()
            if len(perdidos) > 0:
                st.warning(f"🚨 Existem vendas para máquinas não cadastradas! Copie estes NS e vincule agora: **{', '.join(perdidos)}**")
            ...
            if not df.empty:
                ...
                c1.metric("Bruto (Total do Dia)", f"R$ {df['bruto_v'].sum():,.2f}")
...
st.sidebar.caption("MJ Soluções v202.0")
