...
if st.session_state.perfil is None:
    ...
    if menu == "👤 Vincular":
        ...
        with st.form("vin"):
            ...
            ns = st.text_area("Digite os NS (separados por vírgula)")
        ...
        if v_res.data:
            ...
            ns_vinc = set(df_m['link'].unique())
            perdidos = ns_venda - ns_vinc
            if perdidos and "" not in perdidos:
                st.warning(f"⚠️ As vendas de R$ 12.340,00 estão aqui! Vincule estes NS agora: **{', '.join(perdidos)}**")
...
st.sidebar.caption("MJ Soluções v184.0")
