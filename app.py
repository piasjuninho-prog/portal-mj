...
def gerar_pdf_cliente(df, total_bruto, total_liquido):
    ...
    for _, r in df.iterrows():
        ...
        pdf.cell(50, 8, f"R$ {float(r['bruto_v']):,.2f}", 1, ln=True)
    return bytes(pdf.output())
            ...
            if v_raw and m_raw:
                ...
                if not df.empty:
                    ...
                    if st.button("📄 Gerar Relatório PDF"):
                        ...
                        st.download_button("📥 Baixar PDF (Sem Lucro)", pdf_b, "extrato.pdf", "application/pdf")
...
st.sidebar.caption("MJ Soluções v128.0")
