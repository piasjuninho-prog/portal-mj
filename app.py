...
if st.session_state.perfil is None:
    ...
    if menu == "🏫 Gestão":
        ...
        with t_lista:
            ...
            if res.data: st.dataframe(pd.DataFrame(res.data)[['nome_fantasia', 'email', 'adquirente', 'nome_plano_ativo']], use_container_width=True)

        with t_excluir:
            res_ex = conn.table("estabelecimentos").select("id, nome_fantasia").execute()
            if res_ex.data:
                cliente_para_excluir = st.selectbox("Selecione o cliente para REMOVER permanentemente:", options=[c['nome_fantasia'] for c in res_ex.data])
                if st.button("🚨 EXCLUIR CLIENTE AGORA", use_container_width=True):
                    conn.table("estabelecimentos").delete().eq("nome_fantasia", cliente_para_excluir).execute()
                    # Também remove vínculos de máquinas dele
                    conn.table("maquinas_ns").delete().eq("nome_lojista", cliente_para_excluir).execute()
                    st.warning(f"Cliente {cliente_para_excluir} removido!")
                    st.rerun()
    ...
    # --- ABA DASHBOARD ---
        ...
        try:
            ...
            if v_raw and m_raw:
                ...
                df = pd.merge(df_v, df_m, left_on='link_key', right_on='ns', how='inner') # Inner: Só mostra quem você vinculou!
                ...
                if st.session_state.perfil == "admin":
                    ...
                    df = df[df['nome_lojista'].isin(st.sidebar.multiselect("Lojistas:", lista, default=lista))]
            ...
            else: st.info("Sincronize as máquinas reais na aba Vincular.")
        except Exception as e: st.error(f"Erro: {e}")

st.sidebar.caption("MJ Soluções v60.0")
