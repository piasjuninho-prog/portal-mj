...
# --- CONSTANTES ---
ORDEM_MODALIDADES = ["pix", "débito", "à vista"] + [f"em {i}x" for i in range(2, 19)]
...
def gerar_pdf_v242(df, data_ref, bruto, liquido, lucro, qtd, perfil):
    ...
    resumo = f" Bruto: RS {bruto:,.2f}  |  Liquido: RS {liquido:,.2f}"
    if perfil == "admin": resumo += f"  |  Lucro: RS {lucro:,.2f}"
    resumo += f"  |  Vendas: {qtd}"
...
if not st.session_state.auth:
    ...
    # --- DASHBOARD (v242) ---
        ...
        st_autorefresh(interval=300000, key="refresh_v242")
        ...
        if v_res.data:
            ...
            if not df_f.empty:
                ...
                c1.metric("Bruto Total", f"R$ {vb:,.2f}")
                c2.metric("Liquido Total", f"R$ {vl:,.2f}")
                c3.metric("Vendas", vq)
                if st.session_state.perfil == "admin":
                    c4.metric("Lucro MJ", f"R$ {vlu:,.2f}")
                
                st.divider()
                try:
                    pdf = gerar_pdf_v242(df_f, data_txt, vb, vl, vlu, vq, st.session_state.perfil)
    ...
    # --- ABA GESTÃO ---
        ...
        t1, t2, t3 = st.tabs(["📋 Editar", "➕ Novo", "🗑️ Excluir"])
        with t1:
            if res_e.data:
                df_e = pd.DataFrame(res_e.data)
                df_ed = st.data_editor(df_e, use_container_width=True, hide_index=True, key="ed_gest", column_config={"id": st.column_config.Column(disabled=True)})
        ...
        with t2:
            with st.form("add"):
                n, e, s = st.text_input("Nome"), st.text_input("Email"), st.text_input("Senha", "12345")
                if st.form_submit_button("CADASTRAR"):
                    conn.table("estabelecimentos").insert({"nome_fantasia": n.upper(), "email": e.lower(), "senha": s}).execute()
                    st.rerun()
        with t3:
            rem = st.selectbox("Lojista:", sorted([l['nome_fantasia'] for l in res_e.data]))
            if st.button("EXCLUIR"):
                conn.table("estabelecimentos").delete().eq("nome_fantasia", rem).execute()
                st.rerun()

    # --- ABA PLANOS (COM CUSTO) ---
    elif menu == "📂 Planos":
        st.title("📂 Planos")
        ...
        with t1:
            if lista_p:
                ps = st.selectbox("Plano:", lista_p, key="v_pl")
        ...
        with t2:
            ...
            taxas_atuais = []
            if modo == "Editar" and nome_f:
                id_f = next(p['id'] for p in res_p.data if p['nome_plano'] == nome_f)
                res_atuais = conn.table("taxas_dos_planos").select("meio, taxa_decimal, custo_decimal").eq("id_plano", id_f).eq("bandeira", band_s).execute()
                taxas_atuais = res_atuais.data

            mods = ["pix"] if band_s == "pix" else ORDEM_MODALIDADES
            df_base = pd.DataFrame({"Meio": mods, "Venda (%)": 0.0, "Custo (%)": 0.0})
            if taxas_atuais:
                df_a = pd.DataFrame(taxas_atuais)
                df_a['Venda (%)'], df_a['Custo (%)'] = df_a['taxa_decimal']*100, df_a['custo_decimal']*100
...
st.sidebar.caption("MJ Soluções v242.0")
