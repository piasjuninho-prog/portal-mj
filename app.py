...
def gerar_pdf(df, total_bruto, total_liquido):
    ...
    for _, r in df.head(100).iterrows():
        ...
        l = str(r.get('lojista_final', ''))[:25]
        pdf.cell(35, 8, d, 1); pdf.cell(60, 8, l, 1); pdf.cell(30, 8, str(r.get('bandeira', '')), 1); pdf.cell(30, 8, f"{float(r.get('bruto_v', 0)):,.2f}", 1); pdf.cell(35, 8, f"{float(r.get('liq_v', 0)):,.2f}", 1, ln=True)
...
if st.session_state.perfil is None:
    ...
    # --- 🏠 DASHBOARD (v118.0 - FIX ERRO BANDEIRA E VINCULO) ---
    elif menu == "🏠 Dashboard":
        ...
        try:
            ...
            if v_raw:
                ...
                # NORMALIZAÇÃO DE CHAVES (Trata o prefixo SPB da InfinitePay)
                df_v['link_key'] = df_v.apply(lambda x: str(x.get('terminal', '')).strip().lstrip('0') if str(x.get('adquirente','')).lower() == 'picpay' else str(x.get('ns','')).strip().upper().replace('SPB', '').replace('S', '')[:13], axis=1)
                df_m['ns_short'] = df_m['ns'].astype(str).str.strip().str.upper().str.lstrip('0').str[:13]
                
                # MERGE LEFT: Nada some da tela!
                ...
                # EVITA DUPLICIDADE DE COLUNA 'BANDEIRA'
                df_t_prep = df_t.rename(columns={'bandeira': 'band_p', 'meio': 'meio_p'})
                df['band_adj'] = df['bandeira'].astype(str).str.strip().str.lower()
                
                df = pd.merge(df, df_t_prep, left_on=['id_p', 'band_adj', 'plano_adj'], right_on=['id_plano', 'band_p', 'meio_p'], how='left')
                ...
                if not df.empty:
                    ...
                    if st.session_state.perfil == "admin": c4.metric("Lucro Real", f"R$ {df['lucro_v'].sum():,.2f}")

                    st.divider()
                    g1, g2 = st.columns(2)
                    with g1: st.subheader("📈 Diário"); st.line_chart(df.groupby(df['data_dt'].dt.date)['bruto_v'].sum())
                    # FIX BANDEIRA: O gráfico agora usa a coluna original da tabela de vendas
                    with g2: st.subheader("💳 Bandeiras"); st.bar_chart(df.groupby('bandeira')['bruto_v'].sum())
                    
                    if st.button("📄 Gerar PDF"):
                        pdf_b = gerar_pdf(df, df['bruto_v'].sum(), df['liq_v'].sum())
                        st.download_button("📥 Baixar PDF", pdf_b, "relatorio.pdf", "application/pdf")
                    st.dataframe(df[['data_venda', 'lojista_final', 'bandeira', 'plano', 'bruto_v', 'liq_v']].sort_index(ascending=False), use_container_width=True)
            else: st.info("Sincronizando...")
        except Exception as e: st.error(f"Erro no Dashboard: {e}")

st.sidebar.caption("MJ Soluções v118.0")
