...
# Conversor de data atualizado para os formatos do PagBank
...
if st.session_state.perfil is None:
    ...
    if menu == "🏫 Gestão":
        ...
        with t2:
            with st.form("cad"):
                n = st.text_input("Nome Fantasia"); e = st.text_input("E-mail"); a = st.selectbox("Adq", ["InfinitePay", "PicPay", "PagBank"])
    ...
    # --- 🏠 DASHBOARD v131.0 (RECALCULADO SEM VIEWS) ---
    elif menu == "🏠 Dashboard":
        st_autorefresh(interval=30000, key="refresh")
        try:
            # Puxa tudo direto das tabelas brutas
            v_data = conn.table("vendas").select("*").execute().data
            m_data = conn.table("maquinas_ns").select("*").execute().data
            p_ref = conn.table("planos_mj").select("id, nome_plano").execute().data
            t_ref = conn.table("taxas_dos_planos").select("*").execute().data

            if v_data:
                df_v = pd.DataFrame(v_data).drop_duplicates(subset=['id'], keep='first')
                df_m = pd.DataFrame(m_data) if m_raw else pd.DataFrame(columns=['ns', 'nome_lojista', 'nome_plano'])
                
                # Normalização de Vínculo
                ...
                # Merge Vendas + Máquinas + Planos
                df = pd.merge(df_v, df_m, left_on='link_key', right_on='ns_short', how='left')
                df_p_ref = pd.DataFrame(p_ref).rename(columns={'id': 'id_p'})
                df = pd.merge(df, df_p_ref, on='nome_plano', how='left')
                
                # Ajuste de Plano e Merge de Taxas
                df['pl_adj'] = df['plano'].astype(str).str.strip().str.lower().replace('crédito', 'à vista').replace('parcelado', 'em 2x')
                df_t_clean = pd.DataFrame(t_ref).drop_duplicates(subset=['id_plano', 'bandeira', 'meio'], keep='last').rename(columns={'bandeira': 'band_p', 'meio': 'meio_p'})
