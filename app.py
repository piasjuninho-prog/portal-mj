...
if st.session_state.perfil is None:
    ...
    if menu == "🏫 Gestão":
        ...
        with tv:
            ...
            if res_p.data:
                ...
                if res_t.data:
                    ...
                    band_view = st.selectbox("Conferir Bandeira:", ORDEM_BANDEIRAS)
        ...
        with t_vin:
            ...
            if res_e.data and res_p.data:
                with st.form("vin"):
                    c = st.selectbox("Cliente", sorted([e['nome_fantasia'] for e in res_e.data])); ns = st.text_input("NS ou Terminal"); pl = st.selectbox("Plano", sorted([p['nome_plano'] for p in res_p.data]))
    ...
    # --- 🏠 DASHBOARD (v107.0 - RESGATE TOTAL) ---
    elif menu == "🏠 Dashboard":
        ...
        try:
            ...
            if v_raw:
                ...
                # MERGE LEFT: Para o Bruto bater sempre!
                ...
                df = pd.merge(df, df_t, left_on=['id_p', 'bandeira_ajustada', 'plano_adj'], right_on=['id_plano', 'bandeira', 'meio'], how='left')
...
st.sidebar.caption("MJ Soluções v107.0")
