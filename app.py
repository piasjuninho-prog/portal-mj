...
if st.session_state.perfil is None:
    ...
    # --- 🏠 DASHBOARD (v110.0 - FIX BANDEIRA E VALORES TOTAIS) ---
    elif menu == "🏠 Dashboard":
        ...
        try:
            ...
            if v_raw and m_raw:
                ...
                # Removida deduplicação por valor para permitir vendas iguais no mesmo minuto (InfinitePay)
                df_v = df_v.drop_duplicates(subset=['id'], keep='first')

                df_m = pd.DataFrame(m_raw); df_p = pd.DataFrame(p_raw); df_t = pd.DataFrame(t_raw)
                
                # Normalização de Chaves
                df_v['link_key'] = df_v.apply(lambda x: str(x.get('terminal', '')).strip().lstrip('0') if str(x.get('adquirente','')).lower() == 'picpay' else str(x.get('ns','')).strip().upper()[:13], axis=1).astype(str)
                df_m['ns_short'] = df_m['ns'].astype(str).str.strip().str.upper().str.lstrip('0').str[:13]
                
                # Merge 1: Identificar Dono
                df = pd.merge(df_v, df_m, left_on='link_key', right_on='ns_short', how='inner')
                
                # Merge 2: Identificar Plano
                df_p['nome_plano_key'] = df_p['nome_plano'].astype(str).str.strip().str.upper()
                df['nome_plano_key'] = df['nome_plano'].astype(str).str.strip().str.upper()
                df = pd.merge(df, df_p.rename(columns={'id':'id_do_plano'}), on='nome_plano_key', how='left')
                
                # Merge 3: Aplicar Taxas (LÓGICA PARA EVITAR ERRO 'BANDEIRA')
                df_t_prep = df_t.rename(columns={'bandeira': 'band_p', 'meio': 'meio_p', 'id_plano': 'id_plano_p'})
                for c in ['band_p', 'meio_p']: df_t_prep[c] = df_t_prep[c].astype(str).str.strip().str.lower()
                
                df['pl_adj'] = df['plano'].astype(str).str.strip().str.lower().replace('crédito', 'à vista')
                df['ba_adj'] = df['bandeira'].astype(str).str.strip().str.lower()
                df['id_p_str'] = df['id_do_plano'].astype(str)
                df_t_prep['id_p_str'] = df_t_prep['id_plano_p'].astype(str)

                df = pd.merge(df, df_t_prep, left_on=['id_p_str', 'ba_adj', 'pl_adj'], right_on=['id_p_str', 'band_p', 'meio_p'], how='left')
                
                df['data_dt'] = df['data_venda'].apply(converter_data_seguro); df = df.dropna(subset=['data_dt'])

                # Filtros
                lista_lj = sorted(df['nome_lojista'].astype(str).unique())
                if st.session_state.perfil == "admin":
                    esc = st.sidebar.multiselect("Lojistas:", lista_lj, default=lista_lj); df = df[df['nome_lojista'].isin(esc)]
                else: df = df[df['nome_lojista'] == st.session_state.usuario]

                d_ini = st.sidebar.date_input("Início", date(2026, 4, 1)); d_fim = st.sidebar.date_input("Fim", date.today())
                ...
                if not df.empty:
                    ...
                    with col_g1:
                        ...
                        st.line_chart(df.groupby(df['data_dt'].dt.date)['bruto_v'].sum())
                    with col_g2:
                        st.subheader("💳 Vendas por Bandeira")
                        # CORREÇÃO DEFINITIVA DO ERRO 'BANDEIRA'
                        st.bar_chart(df.groupby('bandeira')['bruto_v'].sum())

                    st.dataframe(df[['data_venda', 'nome_lojista', 'bandeira', 'plano', 'bruto_v', 'liq']].sort_index(ascending=False), use_container_width=True)
            else: st.info("Sincronize os vínculos.")
        except Exception as e: st.error(f"Erro no Dashboard: {e}")

st.sidebar.caption("MJ Soluções v110.0")
