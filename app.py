...
if st.session_state.perfil is None:
    ...
    u = st.text_input("E-mail de Login").lower().strip()
    ...
    if menu == "🏫 Gestão":
        ...
        t_lista, t_novo, t_excluir = st.tabs(["📋 Lista de Clientes", "➕ Novo Cadastro", "🗑️ Excluir"])
        
        with t_novo:
            with st.form("form_cad_real", clear_on_submit=True):
                c1, c2 = st.columns(2)
        ...
        with t_excluir:
            ...
            if res_ex.data:
                nome_del = st.selectbox("Escolha o cliente para deletar:", [c['nome_fantasia'] for c in res_ex.data])
                if st.button("🚨 EXCLUIR PERMANENTEMENTE"):
                    conn.table("estabelecimentos").delete().eq("nome_fantasia", nome_del).execute()
                    conn.table("maquinas_ns").delete().eq("nome_lojista", nome_del).execute()
                    st.warning("Cliente removido."); st.rerun()

    # --- 📂 ABA PLANOS (COM EXCLUIR PLANO) ---
    elif menu == "📂 Planos":
        st.title("📂 Planos de Taxas")
        tv, tn, tx = st.tabs(["📋 Meus Planos", "➕ Criar Novo", "🗑️ Deletar Plano"])
        ...
        with tx:
            res_px = conn.table("planos_mj").select("nome_plano").execute()
            if res_px.data:
                plano_del = st.selectbox("Plano para remover:", [p['nome_plano'] for p in res_px.data])
                if st.button("🚨 EXCLUIR PLANO"):
                    conn.table("planos_mj").delete().eq("nome_plano", plano_del).execute()
                    st.success("Plano excluído!"); st.rerun()

    # --- 👤 ABA VINCULAR ---
    elif menu == "👤 Vincular":
        st.title("👤 Vínculo Máquina x Cliente")
        ...
        if res_e.data and res_p.data:
            with st.form("vin"):
                ...
                ns = st.text_input("Código NS ou Terminal")
    ...
    # --- 🏠 DASHBOARD ---
        ...
        try:
            ...
            if v_raw and m_raw:
                df_v = pd.DataFrame(v_raw); df_m = pd.DataFrame(m_raw)
                ...
                df = pd.merge(df_v, df_m, left_on='link_key_short', right_on='ns_short', how='inner')
            ...
            else: st.info("Sincronize as máquinas reais para ver os dados.")
        except Exception as e: st.error(f"Aguardando dados...")

st.sidebar.caption("MJ Soluções v81.0")
