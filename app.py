...
if st.session_state.perfil is None:
    ...
    # --- 🏠 DASHBOARD (v98.0 - LIMPEZA TOTAL) ---
    if menu == "🏠 Dashboard":
        st_autorefresh(interval=30000, key="refresh")
        try:
            # 1. Busca Vendas e Vínculos
            ...
            if v_raw:
                ...
                # DEDUPLICAÇÃO PELO ID DA TRANSAÇÃO (ns)
                df_v = df_v.drop_duplicates(subset=['ns'], keep='first')
                
                # Normalização
                df_v['bruto_num'] = pd.to_numeric(df_v['bruto'], errors='coerce').fillna(0)
                df_v['data_dt'] = df_v['data_venda'].apply(converter_data_seguro)
                df_v = df_v.dropna(subset=['data_dt'])

                # Vínculo com Lojista e Taxas
                ...
                df['lojista_final'] = df['nome_lojista'].fillna('NÃO VINCULADO')

                # Merge Taxas
                df_p = pd.DataFrame(p_raw).rename(columns={'id': 'id_p'})
                df = pd.merge(df, df_p, on='nome_plano', how='left')
                df_t = pd.DataFrame(t_raw).rename(columns={'bandeira': 'band_p', 'meio': 'meio_p'})
                ...
                df = pd.merge(df, df_t, left_on=['id_p', 'band_adj', 'plano_adj'], right_on=['id_plano', 'band_p', 'meio_p'], how='left')

                # Filtros
                st.sidebar.subheader("Filtros")
                l_filt = sorted(df['lojista_final'].unique())
                if st.session_state.perfil == "admin":
                    esc = st.sidebar.multiselect("Lojistas:", l_filt, default=[x for x in l_filt if "VINCULADO" not in x])
                    df = df[df['lojista_final'].isin(esc)]
                else: df = df[df['lojista_final'] == st.session_state.usuario]

                d_ini = st.sidebar.date_input("Início", date(2026, 5, 30)); d_fim = st.sidebar.date_input("Fim", datetime.now().date())
                df = df[(df['data_dt'].dt.date >= d_ini) & (df['data_dt'].dt.date <= d_fim)]

                if not df.empty:
                    df['t_cli'] = pd.to_numeric(df['taxa_decimal'], errors='coerce').fillna(0.0)
                    df['liq_val'] = (df['bruto_num'] * (1 - df['t_cli'])).round(2)
                    df['taxa_pct'] = (df['t_cli'] * 100).map("{:.2f}%".format)

                    st.title("📊 Dashboard Geral")
                    c1, c2, c3 = st.columns(3)
                    c1.metric("Bruto Total", f"R$ {df['bruto_num'].sum():,.2f}")
                    c2.metric("Líquido Total", f"R$ {df['liq_val'].sum():,.2f}")
                    c3.metric("Vendas", len(df))
                    st.divider()
                    st.dataframe(df[['data_venda', 'lojista_final', 'bandeira', 'plano', 'bruto_num', 'taxa_pct', 'liq_val']].sort_index(ascending=False), use_container_width=True)
