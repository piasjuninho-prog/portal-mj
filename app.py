...
def gerar_pdf(df, total_bruto, lucro):
    ...
    pdf.cell(95, 10, f"Liquido Total: R$ {total_bruto-lucro:,.2f}", 1, ln=True)
    pdf.ln(5)
    ...
    if menu == "🏫 Gestão":
        ...
        with tv:
            ...
            if res_p.data:
                ...
                if res_t.data:
                    df_res = pd.DataFrame(res_t.data)
                    band_view = st.selectbox("Bandeira:", ORDEM_BANDEIRAS)
                    df_f = df_res[df_res['bandeira'] == band_view].copy().set_index('meio').reindex(ORDEM_MODALIDADES)
                    df_f['Taxa Cliente'] = (df_f['taxa_decimal'] * 100).map("{:.2f}%".format)
                    df_f['Custo Adq'] = (df_f['custo_decimal'] * 100).map("{:.2f}%".format)
                    st.dataframe(df_f[['Taxa Cliente', 'Custo Adq']], use_container_width=True)
        with tn:
            nome_p = st.text_input("Nome do Plano")
            band_s = st.selectbox("Bandeira:", ORDEM_BANDEIRAS)
            # AQUI: Usuário digita em número inteiro (Ex: 10.19)
        ...
        with st.form("vin"):
            c = st.selectbox("Cliente", [e['nome_fantasia'] for e in res_e.data]); ns = st.text_input("NS/Terminal"); pl = st.selectbox("Plano", [p['nome_plano'] for p in res_p.data])
        ...
        try:
            ...
            if v_raw:
                ...
                if not df.empty:
                    ...
                    if st.session_state.perfil == "admin": c4.metric("Lucro Real (Spread)", f"R$ {df['lucro_real'].sum():,.2f}")

                    st.divider()
                    g1, g2 = st.columns(2)
                    with g1: st.subheader("📈 Faturamento Diario"); st.line_chart(df.groupby(df['data_dt'].dt.date)['bruto_v'].sum())
                    with g2: st.subheader("💳 Vendas por Bandeira"); st.bar_chart(df.groupby('bandeira')['bruto_v'].sum())
...
st.sidebar.caption("MJ Soluções v111.0")
