...
if not st.session_state.auth:
    ...
    # --- ABA DASHBOARD (v202.0 - FIX 60 VENDAS) ---
    elif menu == "🏠 Dashboard":
        ...
        if v_res.data:
            ...
            # --- LEFT JOIN: MOSTRA TUDO, MESMO SEM DONO ---
            ...
            # Alerta de Máquinas perdidas
            perdidos = df[df['nome_lojista'] == "⚠️ MÁQUINA NÃO VINCULADA"]['link'].unique()
            if len(perdidos) > 0:
                st.warning(f"🚨 Existem vendas para NS não cadastrados: **{', '.join(perdidos)}**")
