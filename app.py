...
if st.session_state.perfil is None:
    ...
    # --- 🏠 DASHBOARD (v117.0 - FIX PREFIXO INFINITE) ---
    elif menu == "🏠 Dashboard":
        ...
        try:
            ...
            if v_raw:
                ...
                # --- A MÁGICA: Remove o 'S' do ID da InfinitePay para bater com o vínculo ---
                ...
                # MERGE LEFT (Não apaga nada da tela!)
                ...
                df = pd.merge(df, df_t, left_on=['id_p', 'ba_adj', 'pl_adj'], right_on=['id_plano', 'bandeira', 'meio'], how='left')
                ...
                if not df.empty:
                    ...
                    st.write("---")
                    st.dataframe(df[['data_venda', 'lojista_final', 'bandeira', 'plano', 'bruto_v', 'taxa_txt', 'liq_v']].sort_index(ascending=False), use_container_width=True)
            else: st.info("Sincronizando...")
        except Exception as e: st.error(f"Erro: {e}")

st.sidebar.caption("MJ Soluções v117.0")
