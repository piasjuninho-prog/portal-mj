...
if st.session_state.perfil is None:
    ...
    # --- 🏠 DASHBOARD (v108.0 - FIX DUPLICADOS E BANDEIRA) ---
    elif menu == "🏠 Dashboard":
        ...
        try:
            ...
            if v_raw and m_raw:
                ...
                # DEDUPLICAÇÃO INTELIGENTE: Só apaga se o ID interno do banco for igual
                ...
                # CORREÇÃO DO ERRO 'BANDEIRA': Remove coluna bandeira da tabela de taxas antes de juntar
                df_t_prep = df_t.drop(columns=['bandeira'], errors='ignore').rename(columns={'meio': 'meio_p'})
                df['bandeira_ajustada'] = df['bandeira'].astype(str).str.strip().str.lower()
                
                # Merge Final
                df = pd.merge(df, df_t_clean if 'df_t_clean' in locals() else df_t_prep, left_on=['id_p', 'bandeira_ajustada', 'plano_adj'], right_on=['id_plano', 'bandeira' if 'df_t_clean' in locals() else 'id_plano', 'meio_p'], how='left')
                
                # Se der erro no merge acima, usamos esta via segura:
                df = pd.merge(df_v, df_m, left_on='link_key', right_on='ns_short', how='inner')
                df = pd.merge(df, df_p, on='nome_plano', how='left')
                df_t_safe = df_t.rename(columns={'bandeira': 'band_p', 'meio': 'meio_p'})
                ...
                df = pd.merge(df, df_t_safe, left_on=['id_p', 'ba_adj', 'pl_adj'], right_on=['id_plano', 'band_p', 'meio_p'], how='left')
...
st.sidebar.caption("MJ Soluções v108.0")
