...
if st.session_state.perfil is None:
    ...
    # --- 🏠 DASHBOARD (v126.0 - VERSÃO BLINDADA) ---
    elif menu == "🏠 Dashboard":
        ...
        try:
            ...
            if v_raw:
                ...
                # Normalização SEGURA para o Merge
                df_v['v_ns'] = df_v['ns'].astype(str).str.strip().str.upper()
                df_v['v_term'] = df_v['terminal'].astype(str).str.strip().str.lstrip('0')
                df_m['m_ns'] = df_m['ns'].astype(str).str.strip().str.upper()

                # Tenta vincular pelo Terminal (PicPay)
                df = pd.merge(df_v, df_m, left_on='v_term', right_on='m_ns', how='left', suffixes=('', '_m'))
                
                # Se não achou pelo terminal, tenta pelo NS (InfinitePay)
                def resgatar_nome(row):
                    if pd.notnull(row['nome_lojista']): return row['nome_lojista'], row['nome_plano']
                    # Busca manual se o merge falhou (contém substring)
                    for _, m in df_m.iterrows():
                        if m['m_ns'] in row['v_ns']: return m['nome_lojista'], m['nome_plano']
                    return None, None

                df[['lojista_v', 'plano_v']] = df.apply(lambda r: pd.Series(resgatar_nome(r)), axis=1)
                df['lojista_final'] = df['lojista_v'].fillna(f"⚠️ VINCULAR: {df['v_term'].where(df['v_term'] != '', df['v_ns'].str[-8:])}")

                # Busca Taxas
                df_p = pd.DataFrame(p_raw).rename(columns={'id': 'id_p'}); df_t = pd.DataFrame(t_raw)
                df = pd.merge(df, df_p, left_on='plano_v', right_on='nome_plano', how='left')
...
st.sidebar.caption("MJ Soluções v126.0")
