...
# --- INICIALIZAÇÃO SEGURA DA MEMÓRIA (RESOLVE O ERRO VERMELHO) ---
...
if not st.session_state.auth:
    ...
    # --- MENU LATERAL (SÓ APARECE SE ESTIVER LOGADO) ---
    ...
    st.sidebar.divider()
    menu = st.sidebar.radio("NAVEGAÇÃO", ["🏠 Dashboard", "👤 Vincular", "🏫 Gestão", "📂 Planos", "🚪 Sair"], key="nav_main_v194")
    ...
    # --- ABA GESTÃO ---
        ...
        with c2:
            with st.expander("🗑️ EXCLUIR"):
                if todos_clientes:
                    excluir = st.selectbox("Escolha para apagar", todos_clientes, key="del_sel")
    ...
    # --- ABA DASHBOARD ---
        ...
        st_autorefresh(interval=60000, key="auto_ref_v194")
        ...
        if v_res.data:
            ...
            df = df[df['nome_lojista'].isin(esc_lojistas) | (df['nome_lojista'] == "⚠️ NÃO VINCULADO")]
            ...
            if not df.empty:
                ...
                if len(perdidos) > 0: st.warning(f"⚠️ Vincule estes NS: {', '.join(perdidos)}")
            else: st.info("Nenhuma venda encontrada.")
st.sidebar.caption("MJ Soluções v194.0")
