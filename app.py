...
if not st.session_state.auth:
    ...
    # ABAS GESTÃO E PLANOS (MANTIDAS DAS VERSÕES ANTERIORES)
    elif menu == "🏫 Gestão":
    ...
    # --- ABA DASHBOARD (v206.0 - MONITOR DE VENDAS PERDIDAS) ---
    elif menu == "🏠 Dashboard":
        st_autorefresh(interval=60000, key="auto_ref_v206")
        st.title("📊 Dashboard")
        d_sel = st.sidebar.date_input("Data do Filtro", date(2026, 8, 10))
        ...
        if v_res.data:
            ...
            # Identifica Vendas que entraram no banco mas NÃO tem dono cadastrado
            df_monitor = pd.merge(df_v, df_m[['link', 'nome_lojista']], on='link', how='left')
            vendas_sem_dono = df_monitor[df_monitor['nome_lojista'].isna()]

            if not vendas_sem_dono.empty:
                st.error(f"🚨 ATENÇÃO: Encontramos {len(vendas_sem_dono)} vendas no banco de dados sem vínculo!")
                st.write("Copie o NS abaixo e cadastre na aba **Vincular** para o valor subir no Dashboard:")
                st.table(vendas_sem_dono[['bruto', 'ns', 'adquirente']])
                st.divider()

            # Cruzamento Principal para o Dashboard
...
st.sidebar.caption("MJ Soluções v206.0")
