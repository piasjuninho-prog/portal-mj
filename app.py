...
def clean_pdf_text(text):
    """Limpa textos para garantir compatibilidade com PDF padrão"""
    if text is None: return ""
    # Remove acentos e converte para texto simples
    nfkd_form = unicodedata.normalize('NFKD', str(text))
    text_clean = "".join([c for c in nfkd_form if not unicodedata.combining(c)])
    # Remove emojis e caracteres especiais que travam o PDF
    return text_clean.encode('ascii', 'ignore').decode('ascii').replace('$', 'RS')
...
def gerar_pdf(df, data_ref, bruto, liquido, qtd):
    ...
    for _, row in df.iterrows():
        ...
        pdf.cell(70, 7, clean_pdf_text(str(row['nome_lojista'])[:30]), 1, 0, 'L')
    ...
    # Retorna o PDF como stream de bytes (O SEGREDO DO DOWNLOAD)
    return pdf.output()
...
if not st.session_state.auth:
    ...
    # --- DASHBOARD (v227) ---
        ...
        st_autorefresh(interval=120000, key="refresh_v227")
        ...
        if v_res.data:
            ...
            if not df_f.empty:
                ...
                # --- BOTÃO DE DOWNLOAD PDF RECONSTRUIDO ---
                st.divider()
                try:
                    ...
                    st.error(f"⚠️ Erro ao gerar PDF: {e}")
    ...
    # ... (Mantenha o código original das outras abas)

st.sidebar.caption("MJ Soluções v227.0")
