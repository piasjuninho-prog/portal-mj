...
if not st.session_state.auth:
    ...
    # --- ABA DASHBOARD ---
        ...
        if v_res.data:
            ...
            # Identifica quais NS existem na tabela de vendas mas NÃO existem na tabela de máquinas
            ...
            if not vendas_sem_dono.empty:
                st.error(f"🚨 ALERTA: Existem {len(vendas_sem_dono)} vendas sem dono para o dia {d_sel.strftime('%d/%m')}!")
                st.write("Copie os Números de Série (NS) abaixo e vincule-os na aba **👤 Vincular**:")
                # Agrupa por NS para facilitar a cópia
                ns_faltando = vendas_sem_dono.groupby('ns').agg({'bruto': 'sum', 'adquirente': 'first'}).reset_index()
                st.table(ns_faltando)
                st.divider()

            # Cruzamento Principal (Inner Join para o Dashboard oficial)
            df = pd.merge(df_v, df_m[['link', 'nome_lojista', 'nome_plano']], on='link', how='inner')

            if not df.empty:
                # Aplica filtro de lojistas na sidebar (se for admin)
