    ...
    # --- 🏠 DASHBOARD (v103.0 - MERGE LIMPO E SEGURO) ---
        ...
        try:
            ...
            if v_raw:
                ...
                d_ini = st.sidebar.date_input("Início", date(2026, 5, 30)); d_fim = st.sidebar.date_input("Fim", date.today())
                ...
                if not df.empty:
                    ...
                    c1.metric("Bruto Total", f"R$ {df['bruto_val'].sum():,.2f}")
                    c2.metric("Líquido Total", f"R$ {df['liq_final'].sum():,.2f}")
                    c3.metric("Vendas", len(df))
                    st.divider()
                    st.dataframe(df[['data_venda', 'lojista_final', 'bandeira', 'plano', 'bruto_val', 'taxa_pct', 'liq_final']].sort_index(ascending=False), use_container_width=True)
